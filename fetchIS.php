<?php
    session_start();
    $username=$_SESSION["usn"];
?>
<head>
<title>add new book</title>
    <table style="width: 98%;">
            <tr>
                    <td style="float: right;">
                        <div style="padding-top:15px ;">
                        <label style="width:auto; padding-right:20px;">Loggin:<?php echo $username;?></label>
                       <a href="homepageA.php"> back </a>
                       </div>
                        </td>
                </tr>
            
    
</table>
<style >
label{
width:120px;
display:inline-block;
}
</style>
<link href ="css/main.css" rel="stylesheet" />

</head>
<body>
    <header id="header" class="fixed-top" style="">
       <div class="logo" style="width:100%;" >
        <table>
            <tr>
                
                <td>
                     <div><img style="height:120px;" src="data:image/png;base64," />
     
            </div>
                    </td>
                <td>
                    <div>
                        <h1 style="font-size: 24px;">   NIRMALA COLLEGE LIBRARY </h1>
                </div>
                    </td>
                </tr>
            
            </table>
       
            
        </div>
   
   
       
       </header> 
       <form method="POST" action="fetchIS_act.php">
       <div class="common">
        <div class="col5 rightalign"> <label>ISSUE ID</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="iid">
         </div>
     </div>
<div class="common">
        <div class="col5 rightalign">  &nbsp;
         </div>
         <div class="col5 leftalign">
            <input type="submit" class="php-form-btn" name="SUBMIT" value="GET ISSUE DETAILS">
         </div>
       
     </div>
  </form>
</body>
</html>
<
<!-- <?php
/*iid=$_POST["iid"];
$bname=$_POST["bname"];
$bid=$_POST["bid"];
$usn=$_POST["usn"];
$idate=$_POST["idate"];
$ddate=$_POST["ddate"];
$iname=$_POST["iname"];*/
// session_start();
$con=mysql_connect("localhost","root","");
if(!$con)
{
 echo"couldn't connect to server".mysql_error($con);
}  
$db=mysql_select_db("project_65",$con);
if(!$db)
{
echo"cannot connect to database".mysql_error($con);
}
$iid=$_POST["iid"];
$q1="select * from issue where iid=$iid";
$result=mysql_query($q1,$con);
while($row=mysql_fetch_array($result))
{
$iid1=$row["iid"];
$bname1=$row["bname"];
$bid2=$row["bid"];
$usn1=$row["usn"];
$idate1=$row["idate"];
$ddate1=$row["ddate"];
$_SESSION["due"]=$ddate1;
$iname1=$row["iname"];
?>
<div id="main">
     <div class="common">
           <center><h1><u> ISSUE$RETURN DETAILS</u></h2></center>
            <center><h2><u>ISSUE DETAILS</u></h2></center>
         </div>
<div class="common">
        <div class="col5 rightalign"> <label>BOOK NAME</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="bname" value="<?php echo $bname1;?>">
         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>BOOK ID</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="bid" value="<?php echo $bid2;?>">
         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>USERNAME</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="usn" value="<?php echo $usn1;?>">
         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>ISSUE DATE</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="idate" value="<?php echo $idate1;?>">
         </div>
     </div>
<div class="common">
        <div class="col5 rightalign"> <label>DUE DATE</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="ddate" value="<?php echo $ddate1;?>">
         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>ISSUED BY</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="iname" value="<?php echo $iname1;?>">
         </div>
         <div class="common">
            <?php
        }
        ?>
           </form>
            <center><h2><u>RETURN DETAILS</u></h2></center>
         </div>
     </div>
     </div>
<form action="return.php" method="POST">
    <div class="common">
        <div class="col5 rightalign"> <label>RETURN ID</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="rid"/>
         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>USER ID</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="userid" />

         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>ROLE</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="checkbox" name="role" value="teacher" />TEACHER
            <input class="php-input" type="checkbox" name="role" value="student" />STUDENT

         </div>
     </div>

<div class="common">
        <div class="col5 rightalign"> <label>RETURN DATE</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="date" name="rdate" />

         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign"> <label>RETURNED TO</label>   
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="text" name="rname" value="<?php echo $username;?>" readonly />

         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign">  
         </div>
         <div class="col5 leftalign">
            <input class="php-input" type="hidden" name="fine" />

         </div>
     </div>
     <div class="common">
        <div class="col5 rightalign">  &nbsp;
         </div>
         <div class="col5 leftalign">
            <input type="submit" class="php-form-btn" name="SUBMIT" value="RETURNED">
         </form>
            
            
         </div>
       
       
       </div>
    
     </div>
 </div>
</body>
</form>
<html>

 -->
